...
def generateMapData(mapfile):
    with open(mapfile, "rb") as f:
        map = read_level(f.read())
        print("Reading map '{}' ({})".format(map.name.decode("utf-8"), os.path.basename(mapfile)))
        ...
        for ((layer, x, y), tile) in map.tiles.items():
            ...
            if tile.has_filth():
                ...
                for orientation in tile.edge_data:
                    if orientation.filth_sprite_set == TileSpriteSet.NONE_0:
                        continue
                    if orientation.filth_spike:
        ...
        for (id, (x, y, entity)) in list(map.entities.items()):
            if isinstance(entity, Enemy):
                output["enemies"] += 1
                output["dust_from_enemies"] += entity.FILTH
